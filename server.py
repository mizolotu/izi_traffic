...
from scapy_utils import ScapyServer
from app_utils import AppServer
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--mode', help='Mode', default='scapy')
    ...
    if args.mode == 'scapy':
        server = ScapyServer(args.iface, args.port, label, tcp_gen_path, http_gen_path, tcp_meta['nmin'][args.traffic], tcp_meta['nmax'][args.traffic], tcp_meta['xmin'], tcp_meta['xmax'], http_meta['xmin'], http_meta['xmax'], debug=args.debug)
        server.serve()
    elif args.mode == 'socket':
        server = SocketServer(args.port, tcp_gen_path, http_gen_path, tcp_meta['xmin'], tcp_meta['xmax'], http_meta['xmin'], http_meta['xmax'])
        server.serve()
    elif args.mode == 'app':
        server = AppServer(args.traffic)
        server.serve()
