    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    for update in range(1, nupdates+1):
        ...
        if update % log_interval == 0 or update == 1:
            ...
            logger.logkv('stats/score', safemean([epinfo['r'] for epinfo in epinfobuf]))
            logger.logkv('stats/steps', safemean([epinfo['L'] for epinfo in epinfobuf]))
            logger.logkv('stats/reward', safemean([epinfo['R'] for epinfo in epinfobuf]))
            logger.logkv('stats/time', safemean([epinfo['T'] for epinfo in epinfobuf]))
    ...
    for item in l:
        if type(item) is list and len(item) > 0:
            ...
            if mean is None:
                ...
                mean += item
    return ','.join([str(x) for x in [int(100*item)/100 for item in (mean / count)]]) if count > 0 else None
